...
/// every call is paginated, per the project brief.
abstract class VendorRepository {
  Future<VendorSearchPage> search({required String query, required int page, int pageSize = 8});
}
...
/// opaque to how search is actually implemented server-side.
class PlaceholderVendorRepository implements VendorRepository {
  ...
  @override
  Future<VendorSearchPage> search({required String query, required int page, int pageSize = 8}) async {
    await _simulateLatency();
    final normalized = query.trim().toLowerCase();
    final filtered = normalized.isEmpty
        ? _all
        : _all.where((v) => v.name.toLowerCase().contains(normalized) || v.city.toLowerCase().contains(normalized)).toList();

    final start = (page - 1) * pageSize;
    if (start >= filtered.length) return const VendorSearchPage(vendors: [], hasMore: false);
    final end = (start + pageSize).clamp(0, filtered.length);
    return VendorSearchPage(vendors: filtered.sublist(start, end), hasMore: end < filtered.length);
  }
}
